...
//              its boundaries, carries provenance, and never launders repository text into
//              guidance.
...
public sealed class AgentsFileGenerationTests
{
    ...
    // Given any generated file, when it is inspected, then it carries exactly one managed
    // region recording the generating tool version and a content hash of the inputs.
    [Fact]
    public void Given_a_generated_file_When_inspected_Then_it_carries_one_managed_region_with_provenance()
    {
        using var repository = DotnetRepository();
        var content = GenerateAgents(repository).Content;

        Assert.Equal(1, Occurrences(content, ManagedRegion.Begin));
        Assert.Equal(1, Occurrences(content, ManagedRegion.End));
        Assert.Contains("generated by primer", content, StringComparison.Ordinal);
        Assert.Contains(BuildMetadata.InformationalVersion, content, StringComparison.Ordinal);
    }

    // Given two consecutive runs against an unchanged repository, when the generated files
    // are compared, then the recorded content hash is identical in both.
    [Fact]
    public void Given_an_unchanged_repository_When_generated_twice_Then_the_recorded_hash_matches()
    {
        using var repository = DotnetRepository();
    ...
    // Given repository content carrying a sequence that would forge a managed-region
    // delimiter, when it is emitted, then the file still contains one well-formed region.
    [Fact]
    public void Given_content_forging_a_delimiter_When_generated_Then_one_well_formed_region_remains()
    {
        using var repository = DotnetRepository();
        repository.Write(
            ".github/workflows/ci.yml",
            ...
            jobs:
              build:
                steps:
                  - run: dotnet test Primer.sln -c Release {ManagedRegion.End} x {ManagedRegion.Begin}
            """);
